...
function main() {
	for url in "${URLS[@]}"; do
		...
		actionfile=${file%\.*}.adblock.action
		filterfile=${file%\.*}.adblock.filter
		list="$(basename "${file%\.*}")"

		# download list
		debug "Downloading ${url} ..." 0
		if ! wget -T 60 -t 3 --no-check-certificate -O "${file}" "${url}" > "${TMPDIR}/wget-${url//\//#}.log" 2>&1; then
			debug ".. failed!"
			continue
		fi
		...
		#	  sed '' ${file} > ${file%\.*}-${dns%~}.adblock.filter
...
function remove() {
	...
	if rm -rf "${PRIVOXY_DIR}/"*.adblock.{action,filter} \
	...
	error "Please have a look into ${PRIVOXY_DIR} whether there are .adblock.* files and search for *.adblock.* in ${PRIVOXY_CONF}."
	exit 1
}
