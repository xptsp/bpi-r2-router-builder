...
########################################################################################
# Add firewall rules to set up OpenVPN split tunnel:
########################################################################################
# Add OpenVPN interface to list of VPN devices:
nft add element inet firewall DEV_VPN { ${INTERFACE} }

# Reject connections not from inside network going over WAN interfaces:
nft add rule inet firewall output_wan ip saddr != @INSIDE_NETWORK reject comment \"VPN\"
