...
if [[ ! "${last_update}" == "${this_update}" ]]; then
	...
	OUT=/etc/openvpn/freevpn.conf
	cat "${file}" | egrep -v "(auth-user-pass|script-security|block-outside-dns)" > ${OUT}
	sed -i "s|;comp-lzo|comp-lzo|g" ${OUT}
...
script-security 2
up /etc/openvpn/vpn_iptables.sh
